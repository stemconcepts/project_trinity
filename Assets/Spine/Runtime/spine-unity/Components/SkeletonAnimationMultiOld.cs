...
namespace Spine.Unity {
	...
	public class SkeletonAnimationMultiOld : MonoBehaviour {
		...
		public List<SkeletonAnimation> GetSkeletonAnimations()
		{
		...
		{
			return AddAnimation(FindAnimation(animationName), loop, delay);
		}

		public TrackEntry AddAnimation(Animation animation, bool loop, float delay)
		{
			if (animation == null) return null;
			...
			if (skeletonAnimation != null)
			{
                if (skeletonAnimation != currentSkeletonAnimation)
                {
					StartCoroutine(WaitDuration(delay, () =>
					{
						SetActiveSkeleton(skeletonAnimation);
						skeletonAnimation.skeleton.SetToSetupPose();
						delay = 0;
				...
				return skeletonAnimation.state.AddAnimation(MainTrackIndex, animation, loop, delay);
			} else
            {
		...
		IEnumerator WaitDuration(float waitTime, System.Action action = null)
		{
