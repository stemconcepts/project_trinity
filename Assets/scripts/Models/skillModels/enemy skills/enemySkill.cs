...
{
    public class enemySkill : SkillModel {
    	//[System.Serializable]
    	...
    	public void AttachStatus( List<StatusModel> singleStatusGroup, Status_Manager targetStatus, float power, float duration ){
    	...
    	public void RunExtraEffect( SkillData data, GameObject caster = null ){
    		switch( ExtraEffect ){
    	...
    	//Run Extra Skill
        public void RunExtraSkill( SkillData data, GameObject caster = null ){
    	...
    	public void SummonCreatures( List<GameObject> targetCreatures ){
            for( int i = 0; i < targetCreatures.Count; i++ ) {
                ...
                if( panel ){
                    ...
                    //creatureData.transform.GetChild(1).GetChild(0).gameObject.name = newCreature.GetComponent<Character_Manager>().role + i.ToString() + "status";
                    //newCreature.gameObject.name = newCreature.GetComponent<Character_Manager>().role + i.ToString();
                    newCreature.GetComponent<Character_Manager>().currentPanel = panel;
                    //newCreature.GetComponent<Character_Manager>().role = newCreature.GetComponent<Character_Manager>().role + i.ToString();
